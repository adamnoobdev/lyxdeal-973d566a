...
serve(async (req) => {
  ...
  try {
    ...
    // Get request body
    const { name, email, phone, address }: CreateAdminRequest = await req.json();
    ...
    if (existingUsers && existingUsers.length > 0) {
      return new Response(
        JSON.stringify({ error: "En administratör med denna e-postadress finns redan" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        }
    ...
    console.log("Creating admin with data:", salonData);
    ...
    if (createSalonError) {
      console.error("Error creating admin:", createSalonError);
      return new Response(
    ...
    console.log("Successfully created admin:", salon);
    return new Response(
      JSON.stringify({
        salon: salon,
