...
export const SalonDebugView = ({
  ...
  return <div className="mb-4 p-3 bg-gray-100 rounded-md">
      ...
      {initialValues.id && <div className="mt-3">
          <SubscriptionUpdateButton salonId={initialValues.id} currentPlan={initialValues.subscriptionPlan || "Baspaket"} currentType={initialValues.subscriptionType || "monthly"} skipSubscription={initialValues.skipSubscription} onSuccess={onSubscriptionUpdated} />
          <p className="text-xs mt-2 text-gray-500">
            Detta anropar API:et direkt för att uppdatera prenumerationen om det inte sparas korrekt.
          </p>
        </div>}
      
      <Button variant="outline" size="sm" onClick={onToggleDebugView} className="mt-2">
        Dölj debugging
      </Button>
    </div>;
};