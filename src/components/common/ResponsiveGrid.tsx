...
const ResponsiveGridComponent = ({ children, className = "" }: ResponsiveGridProps) => {
  ...
  return (
    <div className={`grid gap-6 grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-4 3xl:grid-cols-5 ${className}`}>
      {children}
    </div>
  );
};
