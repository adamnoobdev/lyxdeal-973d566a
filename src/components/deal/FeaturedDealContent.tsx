      ...
      <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-6 md:p-8 space-y-2 sm:space-y-4">
        <div className="mb-2 sm:mb-4 flex flex-wrap items-center gap-1.5 sm:gap-2">
          <CategoryBadge 
            category={category} 
            variant="outline" 
            className="border-white/80 text-white backdrop-blur-sm bg-white/10 shadow-sm text-[10px] sm:text-xs py-0.5 px-2 sm:px-2" 
          />
          {isNew && (
            <CategoryBadge
              category="NYTT"
              variant="default"
              className="bg-yellow-400 text-yellow-950 border-transparent text-[10px] sm:text-xs py-0.5 px-2 sm:px-2"
            />
          )}
        </div>
