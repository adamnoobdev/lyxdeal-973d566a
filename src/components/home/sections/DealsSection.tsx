...
const DealsSectionComponent = ({
  ...
  return <div className="space-y-8 md:space-y-12 px-4 md:px-0">
      <section className="space-y-4 md:space-y-6">
        ...
        <div className="space-y-12">
          {CITIES.filter(city => city !== "Alla Städer").map(city => <div key={city} className="space-y-4">
...
const CityDeals = memo(({
  ...
  if (isLoading) {
    return <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
        {[...Array(4)].map((_, i) => <div key={i} className="h-48 md:h-64 bg-accent/5 rounded-lg animate-pulse" />)}
      </div>;
  }
