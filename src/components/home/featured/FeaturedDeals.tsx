...
const FeaturedDealsComponent = () => {
  ...
  if (isLoading) {
    return (
      <ResponsiveGrid columns="grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4" gap="gap-4">
        {[...Array(5)].map((_, i) => (
          <div key={i} className="h-48 sm:h-56 md:h-64 bg-accent/50 rounded-xl animate-pulse" />
        ))}
      </ResponsiveGrid>
    );
  }
